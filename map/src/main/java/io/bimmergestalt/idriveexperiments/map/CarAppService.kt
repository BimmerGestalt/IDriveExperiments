...
class CarAppService: Service() {
	companion object {
		val ACTION_UICONNECTED = "io.bimmergestalt.idriveexperiments.map.uiConnected"
	...
	fun tryStart() {
		if (carConnected.isConnected && uiConnected) {
			...
			MainViewModel.log("Waiting for connection: car(${carConnected.isConnected}) ui($uiConnected)")
		}
	}
	...
	fun startThread() {
		...
		if (iDriveConnectionStatus.isConnected &&
			...
			MainViewModel.log("Starting car app thread")
			thread = CarThread("Map") {
				Log.i(TAG, "CarThread is ready, starting CarApp")

				app = CarApp(
					iDriveConnectionStatus,
					securityAccess,
					CarAppAssetResources(applicationContext, "bmwone"),
				)
			}
			thread?.start()
